...
- 加强了包名的校验规则
- 构架的子包在使用 eslint 时能够将 `scripts/xxx` 文件也含括在内
- 在构建 `eslint.config.js` 时，传入的变量忘了添加双引号转化为实际使用字符串
- 添加了上一版本本该被添加的 `rollup-plugin-typescript2` 插件，该插件解决官方插件 `@rollup/plugin-typescript` 在工作区报错的问题
- 修补了作为子包安装时 `eslint.config.js` 配置文件缺失的问题

## 1.2.0 (2026-1-30)

- 优化了部分逻辑
  - 包管理器选择做了逻辑优化
  - 如果是在工作区下，默认不提示直接按照工作区（不携带根）的做法
  - 如果不是工作区（已有可能是误判），且输入包名携带域（`@`），则问讯使用者是否构建工作区
- **`tsconfig.base.json`** :
  - 重写了这部分内容，并添加了注释
  - 对应调整了 `tsconfig.build.json` 的文件内容
- **`eslint.config.js`**
  - 添加了 `tsconfig.json` 文件开启 `compilerOptions.verbatimModuleSyntax` 后自动修复
